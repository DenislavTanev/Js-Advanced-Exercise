class Parking {
    ...
    removeCar(carNumber) {
        ...
        if (!this.vehicles[carIndex].paid) {
    ...
    getStatistics(carNumber) {
        if(!carNumber){
            return [
                ...
                .map(car => `${car.carModel} == ${car.carNumber} - ${car.paid ? 'Has payed': 'Not payed'}`)
                .join(`\n`)
            ].join(`\n`);
        }

        let car = this.vehicles.find(car => car.carNumber === carNumber);
        return `${car.carModel} == ${car.carNumber} - ${car.paid ? 'Has payed': 'Not payed'}`;
    }
}
